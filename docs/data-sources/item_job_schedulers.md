...
- `local_time_zone_id` (String) The time zone identifier used to interpret the schedule times. Use a Windows time zone identifier, like `Central Standard Time`, `UTC` or `W. Europe Standard Time`. See [Default Time Zones](https://learn.microsoft.com/windows-hardware/manufacture/desktop/default-time-zones) for the list of allowed values.
