...
- `job_type` (String) <i style="color:red;font-weight: bold">(ForceNew)</i> The job type. Allowed job types per item type: dataflow: {ApplyChanges, Execute}; datapipeline: {Execute}; lakehouse: {RefreshMaterializedLakeViews}.
...
- `local_time_zone_id` (String) The time zone identifier used to interpret the schedule times. Use a Windows time zone identifier, like `Central Standard Time`, `UTC` or `W. Europe Standard Time`. See [Default Time Zones](https://learn.microsoft.com/windows-hardware/manufacture/desktop/default-time-zones) for the list of allowed values. String length must be at least 1.
