...
func TestUnit_DynamicToJSON(t *testing.T) {
	...
	expect := `
{
	...
	b, err := typeutils.DynamicToJSON(input)
	require.NoError(t, err)
	require.JSONEq(t, expect, string(b))
}

func TestUnit_JSONToDynamic(t *testing.T) {
	cases := []struct {
		name   string
		input  string
...
func TestUnit_JSONToDynamicImplied(t *testing.T) {
	cases := []struct {
		name   string
		input  string
		expect types.Dynamic
			...
			input: `
{
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			actual, err := typeutils.JSONToDynamicImplied([]byte(tt.input))
			require.NoError(t, err)
			require.Equal(t, tt.expect, actual)
		})
	}
