	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"values": schema.SetNestedAttribute{
				...
				Computed:            true,
				CustomType:          supertypes.NewSetNestedObjectTypeOf[baseShortcutModel](ctx),
...
func (d *dataSourceShortcuts) list(ctx context.Context, model *dataSourceShortcutsModel) diag.Diagnostics {
	...
	respList, err := d.client.ListShortcuts(ctx, model.WorkspaceID.ValueString(), model.ItemID.ValueString(), nil)
	...
	return model.setValues(ctx, model.WorkspaceID.ValueString(), model.ItemID.ValueString(), respList)
}
