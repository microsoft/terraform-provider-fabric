...
func TestUnit_MirroredCatalogDataSource(t *testing.T) {
	workspaceID := testhelp.RandomUUID()
	entity := fakes.NewRandomMirroredCatalogWithWorkspace(workspaceID)

	fakes.FakeServer.Upsert(fakes.NewRandomMirroredCatalogWithWorkspace(workspaceID))
	...
	resource.ParallelTest(t, testhelp.NewTestUnitCase(t, nil, fakes.FakeServer.ServerFactory, nil, []resource.TestStep{
		...
		// error - missing one of required attributes
		{
			Config: at.CompileConfig(
				testDataSourceItemHeader,
				map[string]any{
		...
		// read by id - success
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttrSet(testDataSourceItemFQN, "properties.connection_id"),
				resource.TestCheckResourceAttrSet(testDataSourceItemFQN, "properties.onelake_tables_path"),
				resource.TestCheckResourceAttrSet(testDataSourceItemFQN, "properties.source_type"),
...
func TestAcc_MirroredCatalogDataSource(t *testing.T) {
	workspace := testhelp.WellKnown()["WorkspaceDS"].(map[string]any)
	workspaceID := workspace["id"].(string)

	entity := testhelp.WellKnown()["MirroredCatalog"].(map[string]any)
	...
	resource.ParallelTest(t, testhelp.NewTestAccCase(t, nil, nil, []resource.TestStep{
		...
		// read by id with definition - success
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttr(testDataSourceItemFQN, "display_name", entityDisplayName),
				resource.TestCheckResourceAttr(testDataSourceItemFQN, "description", entityDescription),
				resource.TestCheckResourceAttrSet(testDataSourceItemFQN, "definition.MirroredCatalogDefinition%2Ejson.content"),
			),
		},
	}))
}
