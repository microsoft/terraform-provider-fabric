...
var testHelperLocals = at.CompileLocalsConfig(map[string]any{
	"path": testhelp.GetFixturesDirPath("mirrored_catalog"),
})
...
func TestUnit_MirroredCatalogResource_ImportState(t *testing.T) {
...
func TestUnit_MirroredCatalogResource_CRUD(t *testing.T) {
	workspaceID := testhelp.RandomUUID()
	entityExist := fakes.NewRandomMirroredCatalogWithWorkspace(workspaceID)
	entityBefore := fakes.NewRandomMirroredCatalogWithWorkspace(workspaceID)
	entityAfter := fakes.NewRandomMirroredCatalogWithWorkspace(workspaceID)
