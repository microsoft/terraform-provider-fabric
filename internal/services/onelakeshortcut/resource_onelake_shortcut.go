...
func (r *resourceOnelakeShortcut) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if resp.Diagnostics.Append(plan.set(ctx, plan.WorkspaceID.ValueString(), plan.ItemID.ValueString(), respCreate.Shortcut)...); resp.Diagnostics.HasError() {
		return
	}
...
func (r *resourceOnelakeShortcut) get(ctx context.Context, model *resourceOneLakeShortcutModel) diag.Diagnostics {
	...
	model.set(ctx, model.WorkspaceID.ValueString(), model.ItemID.String(), respGet.Shortcut)
