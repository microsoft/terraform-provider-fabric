...
func TestUnit_OperationsAgentDataSource(t *testing.T) {
...
func TestAcc_OperationsAgentDataSource(t *testing.T) {
	...
	entity := testhelp.WellKnown()["OperationsAgent"].(map[string]any)
	...
	resource.ParallelTest(t, testhelp.NewTestAccCase(t, &testDataSourceItemFQN, nil, []resource.TestStep{
		// read by id
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttrSet(testDataSourceItemFQN, "properties.state"),
			),
		},
		// read by id - not found
		{
		...
		// read by id with definition
		{
			...
			ConfigStateChecks: []statecheck.StateCheck{
				statecheck.ExpectKnownValue(testDataSourceItemFQN, tfjsonpath.New("definition").AtMapKey("Configurations.json").AtMapKey("content"), knownvalue.NotNull()),
			},
		},
	}))
}
