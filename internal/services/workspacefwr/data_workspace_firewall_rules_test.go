...
func TestUnit_WorkspaceFirewallRulesDataSource(t *testing.T) {
		...
		// read
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckTypeSetElemNestedAttrs(testDataSourceItemFQN, "rules.*", map[string]string{
					...
					"value":        *entity.Rules[0].Value,
				}),
				resource.TestCheckTypeSetElemNestedAttrs(testDataSourceItemFQN, "rules.*", map[string]string{
					"display_name": *entity.Rules[1].DisplayName,
					"value":        *entity.Rules[1].Value,
				}),
			),
		},
	}))
...
func TestUnit_WorkspaceFirewallRulesDataSource_Error(t *testing.T) {
	fakeServer := fakes.NewFakeServer()
	fakeServer.ServerFactory.Core.WorkspacesServer.GetFirewallRules = fakeGetFirewallRulesError()
	...
	resource.ParallelTest(t, testhelp.NewTestUnitCase(t, nil, fakeServer.ServerFactory, nil, []resource.TestStep{
		{
			...
			ExpectError: regexp.MustCompile(errMsgGetFirewallRules),
		},
	}))
}
