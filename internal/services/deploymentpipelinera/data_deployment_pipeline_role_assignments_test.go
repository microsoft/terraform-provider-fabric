...
func TestUnit_DeploymentPipelineRoleAssignmentsDataSource(t *testing.T) {
	...
	fakes.FakeServer.ServerFactory.Core.DeploymentPipelinesServer.NewListDeploymentPipelineRoleAssignmentsPager = fakeDeploymentPipelineRoleAssignments(deploymentPipelineRoleAssignments)

	entity := deploymentPipelineRoleAssignments.Value[1]
...
func TestAcc_DeploymentPipelineRoleAssignmentsDataSource(t *testing.T) {
	...
	resource.ParallelTest(t, testhelp.NewTestAccCase(t, nil, nil, []resource.TestStep{
		// read
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				resource.TestCheckResourceAttr(testDataSourceItemsFQN, "deployment_pipeline_id", deploymentPipelineID),
				resource.TestCheckResourceAttrSet(testDataSourceItemsFQN, "values.0.role"),
			),
		},
	},
	))
