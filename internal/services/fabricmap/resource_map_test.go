...
var testHelperLocals = at.CompileLocalsConfig(map[string]any{
	"path": testhelp.GetFixturesDirPath("fabricmap"),
})
...
func TestUnit_MapResource_ImportState(t *testing.T) {
...
func TestUnit_MapResource_CRUD(t *testing.T) {
	workspaceID := testhelp.RandomUUID()
...
func TestAcc_MapResource_CRUD(t *testing.T) {
...
func TestAcc_MapDefinitionResource_CRUD(t *testing.T) {
	workspace := testhelp.WellKnown()["WorkspaceRS"].(map[string]any)
	workspaceID := workspace["id"].(string)
	...
	entityUpdateDisplayName := testhelp.RandomName()
	entityUpdateDescription := testhelp.RandomName()
	lakehouseResourceHCL1, lakehouseResourceFQN1 := lakehouseResource(t, workspaceID)
	lakehouseResourceHCL2, lakehouseResourceFQN2 := lakehouseResource(t, workspaceID)

	testHelperDefinition := map[string]any{
		`"map.json"`: map[string]any{
			"source": "${local.path}/map.json.tmpl",
			"tokens": map[string]any{
				"LAKEHOUSES": fmt.Sprintf(`{"workspaceId": "%s", "artifactId": "%s"}`, workspaceID, testhelp.RefByFQN(lakehouseResourceFQN1, "id")),
			},
		},
	}
	...
	testHelperDefinitionUpdate := map[string]any{
		`"map.json"`: map[string]any{
			"source": "${local.path}/map.json.tmpl",
			"tokens": map[string]any{
				"LAKEHOUSES": fmt.Sprintf(`{"workspaceId": "%s", "artifactId": "%s"}`, workspaceID, testhelp.RefByFQN(lakehouseResourceFQN2, "id")),
			},
		},
	}
