...
func TestUnit_GraphQLApiDataSource(t *testing.T) {
...
func TestAcc_GraphQLApiDataSource(t *testing.T) {
	workspace := testhelp.WellKnown()["WorkspaceDS"].(map[string]any)
	workspaceID := workspace["id"].(string)

	entity := testhelp.WellKnown()["GraphQLApi"].(map[string]any)
	...
	resource.ParallelTest(t, testhelp.NewTestAccCase(t, nil, nil, []resource.TestStep{
		...
		// read by id with definition
		{
			...
			ConfigStateChecks: []statecheck.StateCheck{
				statecheck.ExpectKnownValue(testDataSourceItemFQN, tfjsonpath.New("definition").AtMapKey("graphql-definition.json").AtMapKey("content"), knownvalue.NotNull()),
			},
		},
	}))
}
